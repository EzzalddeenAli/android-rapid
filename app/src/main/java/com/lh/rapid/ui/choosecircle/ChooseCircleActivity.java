 ...
 * 地图选点
 */
public class ChooseCircleActivity extends BaseActivity implements ChooseCircleContract.View {
    ...
    @Inject
    ChooseCirclePresenter mPresenter;
    ...
    private void initRecyclerView() {
        mLinearLayoutManager = new LinearLayoutManager(ChooseCircleActivity.this);
        mRecyclerViewGeocoder.setLayoutManager(mLinearLayoutManager);
        mCommonAdapterGeocoder = new CommonAdapter<HomeCircleBean>(ChooseCircleActivity.this, R.layout.item_aroundmap, mHomeCircleBeans) {
            @Override
            protected void convert(ViewHolder holder, final HomeCircleBean homeCircleBean, int position) {

                holder.setText(R.id.item_aroundmap_name, homeCircleBean.getCircleName());
                holder.setText(R.id.item_aroundmap_address, homeCircleBean.getAddress());
        ...
        mCommonAdapterGeocoder.setOnItemClickListener(new MultiItemTypeAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, RecyclerView.ViewHolder holder, int position) {
                List<HomeCircleBean> datas = mCommonAdapterGeocoder.getDatas();
                if (datas != null) {
                    if (datas != null && datas.size() > position) {
                        setResult(Constants.RESULT_CHOOSE_CIRCLE_CODE, new Intent()
                                .putExtra("name", datas.get(position).getCircleName())
                                .putExtra("id", datas.get(position).getCircleId()));
                    }
                }
                finish();
            }
        ...
        mRecyclerViewGeocoder.addItemDecoration(new DividerGridItemDecoration(ChooseCircleActivity.this, 0));
    ...
    public void initUiAndListener() {
        ...
        mSearchView.setListener(new CustomSearchView.CustomSearchViewListener() {
            ...
            public void onRightButtonClicked() {
                CommonUtils.hideSoftInput(ChooseCircleActivity.this);
                //TODO 跳转选择城市页面
            }
    ...
    private void getCurrentData(LatLng latLng) {
        ...
        mPresenter.homeCircle(latLng.longitude, latLng.latitude);
    }
    ...
    private void showResultPage() {
        ...
        if (mCommonAdapterSearch == null) {
            mListResult.setLayoutManager(new LinearLayoutManager(ChooseCircleActivity.this));
            mCommonAdapterSearch = new CommonAdapter<SearchItem>(ChooseCircleActivity.this, R.layout.location_search_listitem, mSearchItems) {
                @Override
            ...
            mCommonAdapterSearch.setOnItemClickListener(new MultiItemTypeAdapter.OnItemClickListener() {
                ...
                public void onItemClick(View view, RecyclerView.ViewHolder holder, int position) {
                    ...
                    if (datas != null) {
                        ...
                        //                            }
                        //                        }
                        mPresenter.homeCircle(datas.get(position).getLng(), datas.get(position).getLat());
                    }
                    //                    finish();
                    mShadeView();
                    MapStatus.Builder builder = new MapStatus.Builder();
                    builder.target(new LatLng(datas.get(position).getLat(),datas.get(position).getLng()));
    ...
    private List<HomeCircleBean> mHomeCircleBeans = new ArrayList<>();

    @Override
    public void homeCircleSuccess(List<HomeCircleBean> homeCircleBeanList) {
        mHomeCircleBeans.clear();
        mHomeCircleBeans.addAll(homeCircleBeanList);
